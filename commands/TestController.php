...
class TestController extends Controller
{
	...
	const SSQ_OPEN_XML = 'http://zx.500.com/static/info/kaijiang/xml/ssq/14081.xml';
	const DLT_OPEN_XML = 'http://zx.500.com/static/info/kaijiang/xml/dlt/14089.xml';
	const PLS_OPEN_XML = 'http://zx.500.com/static/info/kaijiang/xml/pls/14206.xml';
	const PLW_OPEN_XML = 'http://zx.500.com/static/info/kaijiang/xml/plw/14206.xml';
	const SD_OPEN_XML  = 'http://zx.500.com/static/info/kaijiang/xml/sd/2014206.xml';
	const QXC_OPEN_XML = 'http://zx.500.com/static/info/kaijiang/xml/qxc/14088.xml';
	const EEXW_OPEN_XML= 'http://zx.500.com/static/info/kaijiang/xml/eexw/13172.xml';
    ...
    public function actionIndex()
    {
    	...
    	while ($i>0){
    		foreach (array_keys(Kaijiang::$keyWordMap) as $v){
    			$this->$v($i);
    			echo $v, " {$i} has been crawled!\n";
    		}
    		$i--;
    	}
    }
        
    public function ssq($i)
    {
    	preg_match('/\/(\d+)\.xml$/',self::SSQ_OPEN_XML,$m);
    	$url = preg_replace('/\/(\d+)\.xml$/','/'.($m[1] - $i).'.xml', self::SSQ_OPEN_XML);
    ...
    public function pls($i)
    {
    	preg_match('/\/(\d+)\.xml$/',self::PLS_OPEN_XML,$m);
    	$url = preg_replace('/\/(\d+)\.xml$/','/'.($m[1] - $i).'.xml', self::PLS_OPEN_XML);
    	$xml = $this->get_xml($url);
    	if ( ! $xml )
	    	return false;
    	$data = json_decode(json_encode($xml), true);
     ...
     * 排列五
     */
    public function plw($i)
    {
    	preg_match('/\/(\d+)\.xml$/',self::PLW_OPEN_XML,$m);
    	$url = preg_replace('/\/(\d+)\.xml$/','/'.($m[1] - $i).'.xml', self::PLW_OPEN_XML);
    	$xml = $this->get_xml($url);
    	if ( ! $xml )
	    	return false;
    	$data = json_decode(json_encode($xml), true);
     ...
     * 大乐透
     */
    public function dlt($i)
    {
    	preg_match('/\/(\d+)\.xml$/',self::DLT_OPEN_XML,$m);
    	$url = preg_replace('/\/(\d+)\.xml$/','/'.($m[1] - $i).'.xml', self::DLT_OPEN_XML);
    	$xml = $this->get_xml($url);
    	if ( ! $xml )
	    	return false;
     ...
     * 3D开奖号码拉取
     */
    public function sd($i)
    {
    	preg_match('/\/(\d+)\.xml$/',self::SD_OPEN_XML,$m);
    	$url = preg_replace('/\/(\d+)\.xml$/','/'.($m[1] - $i).'.xml', self::SD_OPEN_XML);
    	$xml = $this->get_xml($url);
    	if ( ! $xml )
	    	return false;
    	$data = json_decode(json_encode($xml), true);
     ...
     * 七星彩
     */
    public function qxc($i)
    {
    	preg_match('/\/(\d+)\.xml$/',self::QXC_OPEN_XML,$m);
    	$url = preg_replace('/\/(\d+)\.xml$/','/'.($m[1] - $i).'.xml', self::QXC_OPEN_XML);
    	$xml = $this->get_xml($url);
    	if ( ! $xml )
	    	return false;
    	$data = json_decode(json_encode($xml), true);
